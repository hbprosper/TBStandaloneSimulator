					    ...
					    const G4double & width)
{
  ...
  std::cout << "== buildSectorStack == " << std::endl;
  std::cout << "sector:           " << sectorNum << std::endl;
  std::cout << "number of modules: " << m_caloStruct.size() << std::endl;
  ...
  for(size_t i=0; i<m_caloStruct.size(); i++)
    {
      ...
      std::cout << "  layer: " << i
		<< "\tcrackOffset: " << crackOffset
		<< "\tangOffset: " << angOffset << std::endl;
	...
	crackOffset=0;
      ...
      std::cout << "  number of elements: " << nEle << std::endl;
	...
	if(thick>0){
	  ...
	  if (sectorNum==0 || sectorNum==m_nSectors-1) {
	    ...
	    G4cout << " layer " << i << " dEdx=" << m_caloStruct[i].ele_dEdx[ie] << " X0=" << m_caloStruct[i].ele_X0[ie] << G4endl
		   << " L0=" << m_caloStruct[i].ele_L0[ie]
		   << "\tzpos=" << m_z0pos+zOverburden << "mm w=" << m_caloStruct[i].ele_thick[ie] << "mm";
	  ...
	  std::cout << "\tpositioning layer at " << xpvpos 
		    << " 0 " 
...
G4VSolid *DetectorConstruction::constructSolid (std::string baseName, G4double thick, G4double zpos,const G4double & minL, const G4double & width){
  ...
  if (model_ == DetectorConstruction::m_FULLSECTION){
    ...
    std::cout << " zpos = " << zpos << " to " << zpos+thick 
	      << " hexagon with side " << m_maxRadius << std::endl;
